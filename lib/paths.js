    ...
    return function(err, stat) {
      if (err) {
        if (!statHanOptions.lstatCalled) {
          var newStatHanOptions = Object.assign(statHanOptions, {lstatCalled:true})
          if(options.sync){
      ...
      var pushVal = statHanOptions.valuetizer(stat, name, statPath)//options.shortName ? name : statPath
    ...
    if(options.shortName){
      if(options.shortName=='relative'){
        statHanOptions.valuetizer = function(stat, shortName, longPath){
          return longPath.substring((options.basePath||dir).length+path.sep.length, longPath.length)
        }
      }else{
        statHanOptions.valuetizer = function(stat, shortName, longPath){
          return shortName
